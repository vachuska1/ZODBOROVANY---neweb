...
export default function StanovyPage() {
  ...
  const [pdfUrl, setPdfUrl] = useState("/api/pdfs/stanovy")

  useEffect(() => {
    fetch("/api/documents/get-url?type=stanovy")
      .then((res) => res.json())
      .then((data) => {
        setPdfUrl(data.url || "/api/pdfs/stanovy")
      })
      .catch(() => {
        setPdfUrl("/api/pdfs/stanovy")
      })
  }, [])
    ...
    <div className="min-h-screen bg-gray-50 py-6">
      <div className="w-[90%] mx-auto">
        <div className="text-center mb-4">
          <h1 className="text-3xl font-bold text-gray-800">{t("statutes")}</h1>
        </div>
